<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&height=180&text=SWENSI™&fontSize=70&color=0:0f0c29,50:302b63,100:24243e&fontColor=fff&animation=fadeIn&fontAlignY=40&desc=Developer%20%2F%20Bot%20Engineer%20%2F%20Coffee%20Addict&descSize=15&descAlignY=62" width="100%"/>

</div>

<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=16&duration=2500&pause=800&color=A78BFA&center=true&vCenter=true&width=600&lines=print(%22Hello%2C+World!%22)+%23+4+years+ago;print(%22Hello%2C+World!%22)+%23+still+doing+this;it+works+on+my+machine+%E2%84%A2;git+push+--force+%23+yolo;undefined+is+not+a+function+%F0%9F%98%AD;have+you+tried+turning+it+off+and+on%3F)](https://git.io/typing-svg)

</div>

<br/>

```python
# /root/swensi17/config.py

class Developer:
    name     = "SWENSI™"
    exp      = "4 года + 1 стаж (считаю с пелёнок)"
    
    stack = {
        "backend":  ["Python", "Node.js", "PHP", "C#", "C++"],
        "frontend": ["React", "TypeScript", "JavaScript"],
        "mobile":   ["Swift (iOS)", "Kotlin/Java (Android)"],
        "infra":    ["Docker", "Nginx", "VPS", "CI/CD"],
        "db":       ["PostgreSQL", "SQLite", "Redis"],
    }
    
    services = [
        "Telegram боты (любые, серьёзно)",
        "Веб-приложения и сайты",
        "iOS / Android приложения",
        "Автоматизация и парсеры",
        "Desktop утилиты",
        "Сдача в срок (не шутка)",
    ]
    
    fun_fact = "баги — это фичи, которые ещё не задокументированы"
    
    def contact(self):
        return "https://t.me/swensidev"
```


---

<div align="center">

### 🛠 Стек

![Python](https://img.shields.io/badge/Python-0d1117?style=flat-square&logo=python&logoColor=3776AB)
![TypeScript](https://img.shields.io/badge/TypeScript-0d1117?style=flat-square&logo=typescript&logoColor=3178C6)
![React](https://img.shields.io/badge/React-0d1117?style=flat-square&logo=react&logoColor=61DAFB)
![Node.js](https://img.shields.io/badge/Node.js-0d1117?style=flat-square&logo=node.js&logoColor=339933)
![C%23](https://img.shields.io/badge/C%23-0d1117?style=flat-square&logo=csharp&logoColor=239120)
![Swift](https://img.shields.io/badge/Swift-0d1117?style=flat-square&logo=swift&logoColor=F05138)
![Kotlin](https://img.shields.io/badge/Kotlin-0d1117?style=flat-square&logo=kotlin&logoColor=7F52FF)
![Docker](https://img.shields.io/badge/Docker-0d1117?style=flat-square&logo=docker&logoColor=2496ED)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-0d1117?style=flat-square&logo=postgresql&logoColor=4169E1)
![Redis](https://img.shields.io/badge/Redis-0d1117?style=flat-square&logo=redis&logoColor=DC382D)
![Nginx](https://img.shields.io/badge/Nginx-0d1117?style=flat-square&logo=nginx&logoColor=009639)
![Linux](https://img.shields.io/badge/Linux-0d1117?style=flat-square&logo=linux&logoColor=FCC624)

</div>

---

<div align="center">

### 📊 GitHub Stats

<img height="155" src="https://github-readme-stats.vercel.app/api?username=swensi17&show_icons=true&theme=midnight-purple&hide_border=true&bg_color=0d1117&title_color=a78bfa&icon_color=a78bfa&text_color=c9d1d9&include_all_commits=true&count_private=true"/>
<img height="155" src="https://github-readme-stats.vercel.app/api/top-langs/?username=swensi17&layout=compact&theme=midnight-purple&hide_border=true&bg_color=0d1117&title_color=a78bfa&text_color=c9d1d9&langs_count=6"/>

[![GitHub Streak](https://streak-stats.demolab.com?user=swensi17&theme=midnight-purple&hide_border=true&background=0d1117&ring=a78bfa&fire=a78bfa&currStreakLabel=a78bfa&sideLabels=c9d1d9&dates=6e7681&card_width=700)](https://git.io/streak-stats)

</div>

---

<div align="center">

### 🚀 Проекты

| | Проект | Описание |
|-|--------|----------|
| 🌐 | [rieltway.uz](https://rieltway.uz) | Legal education platform — Duolingo для юристов |
| 🤖 | [@Post_FF_bot](https://t.me/Post_FF_bot) | Telegram бот публикации игровых постов |

</div>

---

<div align="center">

[![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=swensi17&theme=react-dark&hide_border=true&bg_color=0d1117&color=a78bfa&line=a78bfa&point=ffffff&area=true)](https://github.com/swensi17)

</div>

---

<div align="center">

<img src="https://media.giphy.com/media/LmNwrBhejkK9EFP504/giphy.gif" width="200"/>

**Нужен разработчик?** → [t.me/swensidev](https://t.me/swensidev)

💜 [Поддержать](https://t.me/tribute/app?startapp=ep_7qdvBb34zk7b1QW4bCTfA7YXRfpCz2PJoV25omHd06QXD9Lw7d)

</div>

<img src="https://capsule-render.vercel.app/api?type=waving&height=100&color=0:0f0c29,50:302b63,100:24243e&section=footer" width="100%"/>
